...
class NgxMaskApplierService {
    ...
    cb = () => { }) {
        ...
        if (maskExpression.startsWith("percent" /* MaskExpression.PERCENT */)) {
            ...
            if (inputValue.indexOf("." /* MaskExpression.DOT */) > 0 &&
                !this.percentage(inputValue.substring(0, inputValue.indexOf("." /* MaskExpression.DOT */)))) {
                let base = inputValue.substring(0, inputValue.indexOf("." /* MaskExpression.DOT */) - 1);
                if (this.allowNegativeNumbers &&
                    inputValue.slice(cursor, cursor + 1) === "-" /* MaskExpression.MINUS */) {
                    base = inputValue.substring(0, inputValue.indexOf("." /* MaskExpression.DOT */));
                }
                // eslint-disable-next-line no-param-reassign
                inputValue = `${base}${inputValue.substring(inputValue.indexOf("." /* MaskExpression.DOT */), inputValue.length)}`;
            }
            let value = '';
...
class NgxMaskService extends NgxMaskApplierService {
    ...
    cb = () => { }) {
        ...
        if (this.hiddenInput !== undefined && !this.writingValue) {
            let actualResult = this.actualValue.split("" /* MaskExpression.EMPTY_STRING */);
